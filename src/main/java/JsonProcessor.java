...
//            pointsChangePlayer2 += 400.0 * player2Breaks70Plus / bestOfFrames;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < seasonFiles.size(); i++) {
            ...
            if (i == 0) {
                writePlayersSnapshot(processor, "players_after_" + seasonName + ".csv");
            }
        }
        ...
        try (Writer writer = new FileWriter("players_final.csv")) {
            ...
            for (Player player : processor.getPlayers().values()) {
                ...
                for (Map<String, Double> snapshot : pointsBySeason.values()) {
                    writer.write("," + snapshot.getOrDefault(player.getName(), 0.0));
                }
                writer.write("\n");
            }
        } catch (IOException e) {
    ...
    private static void writePlayersSnapshot(JsonProcessor processor, String fileName) {
